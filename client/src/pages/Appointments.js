...
// ]
 ...
 const normalizedData = () => {
    ...
    return apptsResponse.data.map(appt=>{
        ...
        let physician = docResponse.data.find((doc)=> doc.id === appt.physician_id)
        let patient = patientsResponse.data.find((pat)=> pat.id === appt.patient_id)
        return ({id:appt.id, date:appt.date, phhysician:physician.name, patient:patient.name})
    })
};
